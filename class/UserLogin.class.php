...
class UserLogin
{
	...
	public function check_userlogin () {
		...
		if ( ! isset( $user_email ) || ! isset( $user_password ) ) {
			...
			// Desconfigura qualquer sessão que possa existir sobre o usuário
			$this->logout();
			$this->login_error = "Os dados não foram enviados corretamente. Tente novamente.";
		...
		$p_sql->bindValue(":email", $user_email);
		$p_sql->execute();
		// Verifica a consulta
		if (!$p_sql){
			$this->logged_in = false;
			...
			// Desconfigura qualquer sessão que possa existir sobre o usuário
			$this->logout();
			$this->login_error = "Login e/ou senha inválido(s).";
		...
		if ( $this->phpass->CheckPassword( $user_password, $fetch['senha'] ) ) {
			...
			if (session_id() != $fetch['session_id'] && ! $post) { 
				...
				// Desconfigura qualquer sessão que possa existir sobre o usuário
				$this->logout();
				$this->login_error = "Erro ao tentar fazer login.";
			...
			if ( $post ) {
				...
				$_SESSION['userdata']['user_email'] = $fetch['email'];
				...
				$sql = "UPDATE usuario SET session_id = :session_id WHERE codigo = :user_id";
				$p_sql = Conexao::getInstance()->prepare($sql);
